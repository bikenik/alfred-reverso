...
module.exports.fetching = async input => {
	...
	const resolveThis = await alfy
		...
		.then(async data => {
			...
			itemsArr.push(ankiInfoRes[0] ? ankiInfoRes[0] : ankiInfoRes)
			for (const x of data.data) {
				x.currentSrc = x.srcLang === srcExist ? trgContext : srcContext
				x.toOddcast = x.srcLang === srcExist ? x.trgContext : x.srcContext

				const item = new Render('list of favorite',
					'title', 'subtitle', 'sentence', 'icon', 'arg', 'valid', 'variables')
				item.title = `${x.srcText}\n ↳ ${x.trgText}`
				item.subtitle = x.largeText.join('\n🔑 ').replace(/<em.*?>|<\/em>/g, '') || x.srcSegment
				item.sentence = x.srcSegment ? `${x.srcSegment} ${x.comment === null || x.comment === '' ? '' : `\n✏️ ${x.comment}\n`} ${x.documentTitle ? `\n\t\t🌍 ${x.documentTitle}` : ''}` : 'not offer target from context'
				item.icon = exampleNotExist(x) ? './icons/red-flag.png' : './icons/flag.png'
				item.arg = x
				item.valid = exampleNotExist(x) ? false : null
				item.variables = {
					action: 'dic',
					mode: 'regular',
			...
			return alfy.matches(input.replace(/^\*/g, ''), itemsArr, 'title')
				.map(x => ({
					title: x.title,
					subtitle: x.subtitle,
